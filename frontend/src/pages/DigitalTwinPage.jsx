...
export default function DigitalTwinPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        console.error('Failed to load digital twin records:', err);
      } finally {
        setLoading(false);
      }
    }
  ...
  if (loading) {
    return (
      <div className="gov-card text-center py-16 space-y-3">
        ...
        <p className="text-[13px] font-mono text-gov-muted">Synthesizing Digital Twin Schema...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <Breadcrumbs items={[{ label: 'Services', path: '/' }, { label: 'Digital Twin' }]} />
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-[24px] font-extrabold text-gov-primary">
              Document Digital Twin Studio
            </h1>
            <p className="text-[14px] text-gov-muted">
              Side-by-side comparison cross-examining physical visual credential scan against extracted structured data and ICAO MRZ payload.
            </p>
          </div>

          <div className="flex items-center space-x-2">
            <label htmlFor="dt-case-select" className="text-[12px] font-bold text-gov-muted font-mono uppercase">
              SELECT RECORD:
            </label>
            <select
              id="dt-case-select"
      ...
      {activeScreening ? (
        <DigitalTwinView
          extractedData={activeScreening.extracted_data}
          mrzData={activeScreening.mrz_data}
          documentImageUrl={activeScreening.document_image_url}
          livePhotoUrl={activeScreening.live_photo_url}
          faceResult={activeScreening.face_result}
        />
      ) : (
